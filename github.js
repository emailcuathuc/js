...
(function (doc) {
  ...
  var a = () => {
      ...
      fetch(n)
        ...
        .then((i) => {
          let n = i.sha,
            ...
            k = t.replace(
              e,
              `https://cdn.jsdelivr.net/gh/$1/$2@${n.substring(0, 7)}/$4`
            ),
            q = t.split("/").pop();
          ...
          if (q.indexOf(".js") != -1) {
            doc.getElementById(
              "n"
            ).value = `<script src="${c}" type="text/javascript"><\/script>`;
            doc.getElementById(
              "p"
            ).value = `<script src="${d}" type="text/javascript"><\/script>`;
            doc.getElementById(
              "t"
            ).value = `<script src="${k}" type="text/javascript"><\/script>`;
          }
          if (q.indexOf(".css") != -1) {
            doc.getElementById(
              "n"
            ).value = `<link href="${c}" rel="stylesheet" type="text/css"/>`;
            doc.getElementById(
              "p"
            ).value = `<link href="${d}" rel="stylesheet" type="text/css"/>`;
            doc.getElementById(
              "t"
            ).value = `<link href="${k}" rel="stylesheet" type="text/css"/>`;
          }
          if (
            q.indexOf(".png") != -1 ||
            q.indexOf(".jpg") != -1 ||
    ...
    giturl.test(t)
      ...
      : rawgit.test(t)
      ? n(rawgit, l)
      : doc.getElementsByClassName("gitlink")[0].classList.add("invalid");
  };

  var b = (id) => {
    doc.getElementById(id).value.length > 0
      ? (doc.getElementById(id).select(), doc.execCommand("copy"))
      : alert("Nothing to copy !");
  };

  var c = (id) => {
    doc.getElementById(id).value.length > 0
      ? window.open(doc.getElementById(id).value)
